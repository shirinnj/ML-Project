...
def calibrate_for_one_date(j):
    ...
    grid=pd.read_csv("../nyc_grid_all.csv")
    ...
    for i in A:
        taxi = pd.read_csv("../Data_2016/Output/{0}/{0}~{1}.csv".format(taxi_file_name,i))
        ...
        df_digitized.to_csv("../Data_2016/Output/{0}/{0}~{1}_digital.csv".format(taxi_file_name,i) , index=None)
        G=G.append(df_digitized)
       
        
        b=df_digitized.groupby(by=['pickup_bin','dropoff_bin']).count()
        Pickup_to_dropoff_count=b['Unnamed: 0']
        Pickup_to_dropoff_count.to_csv("..Count/pick up to dropoff trips - 2016/P_to_d_count_{0}~{1}.csv".format(taxi_file_name,i),header=True)
        ...
        Pickup_sum=a[i]
        Pickup_sum.to_csv("..Count/pickup count - 2016/Pickup_sum_"+taxi_file_name+'~'+i+'.csv',header=True)
    ...
    Daily[date_time_cols].to_csv("../Count/Daily_count - 2016/Daily_"+taxi_file_name+'count.csv'
